...
export default function BottomBar({
  ...
  return (
    <div className="w-full max-w-md px-2 mt-1 mb-1 flex items-center gap-2 relative z-10" id="bottom-control-bar-container">
      {/* CLEAR ROW BUTTON (TEMİZLE) */}
      <button
        onClick={onClear}
        disabled={isClearDisabled}
        className={`flex-1 py-1.5 px-3 rounded-lg font-black text-xs sm:text-sm uppercase tracking-wider shadow-md transition-all duration-200 flex items-center justify-center gap-1.5 border ${
          !isClearDisabled
      ...
      <button
        ...
        className={`flex-[2] py-1.5 px-4 rounded-lg font-black text-xs sm:text-sm uppercase tracking-widest shadow-md transition-all duration-200 flex items-center justify-center gap-2 border ${
          !isSubmitDisabled
